...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		noModsTxt = new FlxText(0, 0, FlxG.width, "没有安装任何Mod！\n请返回主界面并安装一个Mod", 48);
		if(FlxG.random.bool(0.1)) noModsTxt.text += '\nBITCH.'; //meanie
		noModsTxt.setFormat(Paths.font("chin.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		buttonToggle.label.setFormat(Paths.font("chin.ttf"), 24, FlxColor.WHITE, CENTER);
		...
		buttonUp.label.setFormat(Paths.font("chin.ttf"), 24, FlxColor.BLACK, CENTER);
		...
		buttonTop.label.setFormat(Paths.font("chin.ttf"), 24, FlxColor.BLACK, CENTER);
		...
		startX -= 190;
		buttonDisableAll = new FlxButton(startX, 0, "DISABLE ALL MOD", function() {
			for (i in modsList)
			{
				i[1] = false;
			}
		...
		buttonDisableAll.label.setFormat(Paths.font("chin.ttf"), 24, FlxColor.BLACK, CENTER);
		...
		startX -= 190;
		buttonEnableAll = new FlxButton(startX, 0, "ENABLE ALL MOD", function() {
			for (i in modsList)
			{
				i[1] = true;
			}
		...
		buttonEnableAll.label.setFormat(Paths.font("chin.ttf"), 24, FlxColor.BLACK, CENTER);
		...
		buttonVerify = new FlxButton(startX, 0, "VERIFY", function() {
			...
			if (modURL == null || modURL.trim() == "") {
				Alert.alert("未提供下载Mod的URL", "其他玩家将无法下载此Mod！\n请 选项 中设置！");
		...
		buttonVerify.label.setFormat(Paths.font("CHIN.ttf"), 24, FlxColor.BLACK, CENTER);
		...
		buttonDelete.label.setFormat(Paths.font("chin.ttf"), 24, FlxColor.BLACK, CENTER);
		...
		*/
		...
		descriptionTxt.setFormat(Paths.font("chin.ttf"), 32, FlxColor.WHITE, LEFT);
		...
		for (mod in mods)
		{
			...
			if(i == curSelected)
			{
				...
				if (mod.restart){//finna make it to where if nothing changed then it won't reset
					descriptionTxt.text += " (这个Mod将会重新启动游戏！)";
				}

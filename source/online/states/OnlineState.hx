...
class OnlineState extends MusicBeatState {
    ...
    override function create() {
        ...
        for (itm in itms) {
            ...
            text.ID = i;
			text.setFormat(Paths.font("chin.ttf"), 27, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			text.alpha = inputWait ? 0.5 : 0.9;
		...
		itemDesc.setFormat(Paths.font("chin.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		itemDesc.screenCenter(X);
		add(itemDesc);

		playersOnline = new FlxText(0, 100);
		playersOnline.setFormat(Paths.font("chin.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		playersOnline.alpha = 0.7;
		playersOnline.text = "正在获取在线人数...";
		playersOnline.screenCenter(X);
		add(playersOnline);

		var availableRooms = new FlxText(0, 130);
		availableRooms.setFormat(Paths.font("chin.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		frontMessage.setFormat(Paths.font("chin.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if (!inputWait) {
			...
			if (FlxG.mouse.justPressed || FlxG.mouse.justMoved) {
				if (FlxG.mouse.overlaps(discord)) {
					...
					itemDesc.text = "加入 Psych Online 的 Discord 服务器";
					itemDesc.screenCenter(X);
				...
				if (twitterIsDead) {
					if (FlxG.mouse.overlaps(bsky)) {
						...
						itemDesc.text = "关注 Psych Online 的 Bluesky 官方帐户！";
						itemDesc.screenCenter(X);
					...
					if (FlxG.mouse.overlaps(twitter)) {
						...
						itemDesc.text = "关注 Psych Online 的 Twitter 官方帐户！";
						itemDesc.screenCenter(X);
	...
	function changeSelection(diffe:Int) {
		...
		switch (curSelected) {
			case 0:
				itemDesc.text = "使用房间号加入房间（无需按该按钮，直接Ctrl + V 即可）";
			case 1:
				itemDesc.text = "创建一个房间";
			case 2:
				itemDesc.text = "查看可加入的公开房间";
			case 3:
				itemDesc.text = "Psych Online 选项，在这里设置！";
			case 4:
				itemDesc.text = "玩家积分排行榜";
			case 5:
				itemDesc.text = "在这里可以下载来自Gamebanana的Mod！";
		}
		itemDesc.screenCenter(X);

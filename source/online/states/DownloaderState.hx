...
class DownloaderState extends MusicBeatState {
	...
	override function create() {
		...
		searchPlaceholder.text = "在这里搜索Mod // 输入Mod的网址(Gamebanana)以下载...";
		searchPlaceholder.setFormat(Paths.font("chin.ttf"), 20, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		searchInput.setFormat(Paths.font("chin.ttf"), 20, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		pageInfo.setFormat(Paths.font("chin.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		pageInfo.y = FlxG.height - pageInfo.height - 30;
		add(pageInfo);

		var pageTip1 = new FlxText(20, 0, FlxG.width, 'Q - 返回上一页');
		pageTip1.setFormat(Paths.font("chin.ttf"), 20, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		pageTip1.y = pageInfo.y;
		pageTip1.alpha = 0.6;
		add(pageTip1);

		var pageTip2 = new FlxText(-20, 0, FlxG.width, 'E - 前往下一页');
		pageTip2.setFormat(Paths.font("chin.ttf"), 20, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				...
				if (curSelected == -1) {
					...
					if (FlxG.mouse.justPressed) {
						if (FlxG.mouse.overlaps(items.members[curSelected].dlBg)) {
							...
							RequestSubstate.requestURL(items.members[curSelected].mod.url, "将跳转至网页:", true);
						}
					}
					else {
						openModDownloads(items.members[curSelected].mod.id);
...
class ModItem extends FlxSpriteGroup {
	...
	public function new(mod:ModInfo) {
		...
		categoryName.setFormat(Paths.font("chin.ttf"), 15, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		name.setFormat(Paths.font("chin.ttf"), 15, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(name);
		...
		likes.setFormat(Paths.font("chin.ttf"), 20, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(likes);

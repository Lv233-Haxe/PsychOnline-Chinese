...
class SkinsState extends MusicBeatState {
    ...
    override function create() {
		...
		charSelect.text = '按击回车以选择！';
		charSelect.setFormat(Paths.font("chin.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		swagText.text = '按着Shift，并按击你绑定的按击以让角色做动作！';
		swagText.setFormat(Paths.font("chin.ttf"), 18, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		swagText.alpha = 0.8;
		swagText.cameras = [hud];
		add(swagText);

		var tip1 = new FlxText(20, 0, FlxG.width, 'TAB - 翻转角色\n8 - 编辑角色\nCTRL - 打开角色列表');
		tip1.setFormat(Paths.font("chin.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		tip1.y = charSelect.y - 10;
		tip1.alpha = 0.5;
		tip1.cameras = [hud];
		add(tip1);

		var tip2 = new FlxText(-20, 0, FlxG.width, 'F1 - 获取帮助\nF2 - 浏览经过验证的皮肤');
		tip2.setFormat(Paths.font("chin.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		tip2.y = tip1.y;
    ...
    override function update(elapsed) {
        ...
        if (controls.ACCEPT) {
			...
			if (isEquiped(charactersMod.get(charactersName[curCharacter]), charName)) {
				charSelect.text = '选择成功！!';
				charSelect.alpha = 1;
			}
			else {
				charSelect.text = '按击 回车 以选择！';
				charSelect.alpha = 0.8;
			}
			if (acceptSound != null)
				acceptSound.stop();

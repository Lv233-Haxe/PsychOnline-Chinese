...
class FindRoomState extends MusicBeatState {
    ...
    override function create() {
		...
		tip = new FlxText(0, 0, 0, '按击 回车键 加入房间');
		tip.setFormat(Paths.font("chin.ttf"), 18, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		emptyMessage = new FlxText(0, 0, FlxG.width, '没有找到任何可用房间:(');
		emptyMessage.setFormat(Paths.font("chin.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function refreshRooms(wLoading:Bool = true) {
		...
		GameClient.getAvailableRooms(GameClient.serverAddress, (err, rooms) -> {
            Waiter.put(() -> {
                ...
                if (err != null) {
					Alert.alert("无法连接！", "ERROR: " + ShitUtil.prettyStatus(err.code) + " - " + err.message + (GameClient.serverAddress.endsWith(".onrender.com") ? "\nTry again in a few minutes! The server is probably restarting!" : ""));
                    return;
                }
...
class RoomBox extends FlxSpriteGroup {
    ...
    public function new(name:String, code:String, pingMs:String, points:Null<Float>, verified:Bool) {
		...
		title.setFormat(Paths.font("chin.ttf"), 22, FlxColor.WHITE, LEFT);
		...
		ping.setFormat(Paths.font("chin.ttf"), 20, FlxColor.WHITE, RIGHT);
		ping.setPosition(10, title.y);
		add(ping);

		detailsTxt = new FlxText(0, 0, bg.width - 20, '> 回车: $code < ');
		detailsTxt.setFormat(Paths.font("chin.ttf"), 20, FlxColor.WHITE, CENTER);
